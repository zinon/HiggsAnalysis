...
//BoolTox::~BoolTox() = default; //pure and auto generated
...
double BoolTox::DeltaRapidity(const xAOD::IParticleContainer *pc){
  if( pc->size()  > 1 )
...
double BoolTox::METcentrality(const xAOD::IParticleContainer *pci, unsigned int i, const xAOD::IParticleContainer *pcj, unsigned int j, const xAOD::MissingET* met){


  if( i < static_cast<unsigned int>(pci->size() ) &&  j < static_cast<unsigned int>(pcj->size() ) )
    return METcentrality( (*pci)[i], (*pcj)[j], met );
  else
    Warning("BoolTox::METcentrality", "elements %i or %i over the range of the input containers... ", i, j );

  return -1234;
}

double BoolTox::METcentrality(const xAOD::IParticleContainer *pc, const xAOD::MissingET* met){


  if( pc->size() > 1  )
    return METcentrality( (*pc)[0], (*pc)[1], met );
  else
    Warning("BoolTox::METcentrality", "too few elements (%lui) for the input container ", pc->size() );

  return -1234;
}

double BoolTox::METcentrality(const xAOD::IParticle *p1, const xAOD::IParticle *p2, const xAOD::MissingET* met){
...
double BoolTox::MT(const xAOD::IParticle *p, const xAOD::MissingET* met){
  
  return std::sqrt( 2 * p->pt() * met->met() * (1 - DeltaPhi( p->phi(), met->phi())) ); 

}

bool BoolTox::MassCollinear(const xAOD::TauJetContainer *pci, unsigned int i, //particle container 
  ...
  if( i >= static_cast<unsigned int>(pci->size()) ){
    Warning("BoolTox::MassCollinear", "element %i exceeds size of tau container ", i );
    return false;
  }
  if( j >= static_cast<unsigned int>(pcj->size()) ){
    Warning("BoolTox::MassCollinear", "element %i exceeds size of muon container ", j );
    return false;
  }
...
bool BoolTox::MassCollinear(const xAOD::TauJetContainer *pc,//particle container 
  ...
  if( pc->size() < 1 ){
    Warning("BoolTox::MassCollinear", "insufficient tau container size %lui", pc->size() );
    return false;
  }
...
bool BoolTox::MassCollinear(const xAOD::MuonContainer *pc,//particle container 
			    const xAOD::MissingET* met, //met
			    const bool kMMCsynchronize, //mmc sychronization
			    double &mass, double &xp1, double &xp2){ //result

  if( pc->size() < 1 ){
    Warning("BoolTox::MassCollinear", "insufficient muon container size %lui", pc->size() );
    return false;
  }
