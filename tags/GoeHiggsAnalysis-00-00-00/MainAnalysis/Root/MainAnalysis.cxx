...
{
  //-------------------------
  // set default jobOptions |
  //-------------------------

  //jets
  jobOptions_jet_min_pt = 15.;
  jobOptions_jet_max_abs_eta = 4.5;
  jobOptions_apply_jet_recalibration = 0;
  jobOptions_apply_JES_correction = 0;
  jobOptions_apply_JER_correction = 0;
  jobOptions_jet_cleaning = "VeryLooseBad"; //VeryLooseBad MediumBad

  //taus
  jobOptions_tau_jet_overal_dR = 0.2;
  jobOptions_tau_muon_overal_dR = 0.2;
  jobOptions_tau_selection_recommended = 1;
  jobOptions_apply_tau_correction = 0;
  jobOptions_tau_min_pt = 20.;
  jobOptions_tau_jet_bdt = "JETIDBDTLOOSE";
  jobOptions_tau_ele_bdt = "ELEIDBDTLOOSE";

  // muons
  jobOptions_apply_muon_calibration_and_smearing = 0;
  jobOptions_muon_min_pt  = 20.0; 
  jobOptions_muon_max_iso = 0.1;  

  //met
  jobOptions_met_container = "MET_RefFinal";

  jobOptions_met_jetColl = "AntiKt4LCTopoCalibJets";
  jobOptions_met_muonColl = "CalibMuons";
  jobOptions_met_tauColl = "CalibTaus";
  jobOptions_met_eleColl = "CalibElectrons";


  jobOptions_met_eleTerm = "RefEle";
  jobOptions_met_gammaTerm = "RefGamma";
  jobOptions_met_tauTerm = "RefTau";
  jobOptions_met_jetTerm = "RefJet"; //RefJet RefJet_JVFCut
  jobOptions_met_muonTerm = "Muons";
  jobOptions_met_softTerm = "SoftClus"; // SoftClus PVSoftTrk

  jobOptions_met_outMETCont = "MET_Calib";
  jobOptions_met_outMETTerm = "Final";
 
  jobOptions_met_jetPtCut = 0.0; //GeV
  jobOptions_met_jetDoJvf = false;
  
  // create reweighting file for input sample
  jobOptions_createPUfile = false;
  jobOptions_testRun = false;
  jobOptions_runGrid = false;
}
...
EL::StatusCode MainAnalysis :: initialize ()
{
  ...
  */
  ...
  // Jet Calibration  ||
  //===================
  std::string   JetCalibSeq = isData ? "JetArea_Residual_AbsoluteEtaJES_Insitu" : "JetArea_Residual_AbsoluteEtaJES";
  std::string   JetCalibConf = "$ROOTCOREBIN/data/JetCalibTools/CalibrationConfigs/JES_Full2012dataset_Preliminary_Jan13.config";
  m_jetCalibTool = new JetCalibrationTool( "MainAnalysis-JetCalibrationTool",
                                           "AntiKt4LCTopo",
                                           JetCalibConf,
                                           JetCalibSeq,
                                           isData);
  ...
  m_JESUncertaintyTool->msg().setLevel( MSG::ERROR );
  m_JESUncertaintyTool->setProperty("JetDefinition","AntiKt4LCTopo");
  m_JESUncertaintyTool->setProperty("MCType","MC12a");
  m_JESUncertaintyTool->setProperty("ConfigFile","JES_2012/Moriond2013/InsituJES2012_14NP.config");
  ...
  // Tau Efficiency Tool  ||
  //=======================
  m_tauEffTool = new TauAnalysisTools::TauEfficiencyCorrectionsTool("TauEfficiencyCorrectionsTool");
  ...
  // m_tauSmearTool->setProperty("Direction",1); //systematics up
  // m_tauSmearTool->setProperty("Direction",-1); //systematics down
  // m_tauSmearTool->setProperty("DataFile","RootCoreBin/data/TauAnalysisTools/Smearing/mc12_p1344_medium.root"); //set data file
  ...
  // TauSelectionTool   ||
  //====================
  m_tauSelTool = new TauAnalysisTools::TauSelectionTool("MainAnalysis-TauSelectionTool");
  m_tauSelTool->msg().setLevel( MSG::DEBUG );
  //control plots
  //m_tauSelTool->setProperty("CreateControlPlots", true );
  //m_tauSelTool->setOutFile( wk()->getOutputFile ("outputLabel") );
  ...
  if(jobOptions_tau_selection_recommended)
    ...
    //define cuts by hand
    //tmp - do not forget to add the right place
  ...
  //define shift range for jet systematics - tmp solution
  CP::SystematicSet* jesshiftSet = new CP::SystematicSet("Peter");
  ...
  jesshiftSet->insert(CP::SystematicVariation("EtaIntercalibration_Modelling",-1.0));
  jesshiftSet->insert(CP::SystematicVariation("EtaIntercalibration_StatAndMethod",1.0));
  jesshiftSet->insert(CP::SystematicVariation("EtaIntercalibration_StatAndMethod",-1.0));
  jesshiftSet->insert(CP::SystematicVariation("RelativeNonClosure_MC12a",1.0));
  jesshiftSet->insert(CP::SystematicVariation("RelativeNonClosure_MC12a",-1.0));
  jesshiftSet->insert(CP::SystematicVariation("SingleParticle_HighP",1.0));
  jesshiftSet->insert(CP::SystematicVariation("SingleParticle_HighP",-1.0));
  ...
  for( const auto &syst : m_syst){
    ...
    m_Tree.insert(std::make_pair(syst,  new TTree(syst.c_str(), syst.c_str()) )  );
    //m_Tree.insert(std::make_pair(syst,  std::shared_ptr< TTree >( new TTree(syst.c_str(), syst.c_str()) ) ) );

  }
  //    m_Tree.insert(std::make_pair(syst,std::unique_ptr<TTree>( new TTree(syst.c_str(), syst.c_str())) ) );//in C++11
  //    m_Tree.insert(std::make_pair(syst, CxxUtils::make_unique<TTree>( new TTree(syst.c_str(), syst.c_str())) ) );//until the C++14 std::make_unique becomes available
  //    m_Tree.emplace( syst, new TTree(syst.c_str(), syst.c_str())  );
...
EL::StatusCode MainAnalysis :: execute ()
{
  ...
  //if(m_eventCounter > 5000) return EL::StatusCode::SUCCESS; //tmp
  m_eventCounter++;
  ...
  if ( !m_event->retrieve( JetCont, "AntiKt4LCTopoJets" ).isSuccess() ){
  ...
  //list of cuts on jets that define the good ones
  std::vector<std::string> vJetGoodFlags { "clean", "pt_cut", "eta_cut" , "no_tau_overlap"};
  ...
  for (std::vector<CP::SystematicSet>::const_iterator sysListItr = m_sysList.begin(); sysListItr < m_sysList.begin() + 1; ++sysListItr){//tmp - just to test nominal only
    ...
    if( m_muonCalibrationAndSmearingTool->applySystematicVariation( *sysListItr ) != CP::SystematicCode::Ok ) {
      ...
      //return EL::StatusCode::FAILURE; //stop
    ...
    for(auto MuonShallowCopyItr = (m_MuonContShallowCopy.first)->begin(); MuonShallowCopyItr != (m_MuonContShallowCopy.first)->end(); MuonShallowCopyItr++) {
      ...
      if(jobOptions_apply_muon_calibration_and_smearing){
        ...
        //  Warning("execute()","Cannot apply shallow-copy muon calibration/smearing out of validity range, eta %.2f", (*MuonShallowCopyItr)->eta());
        //}
      }

      //Efficiency SF
      float effSF = 1.0;
      CP::CorrectionCode mucorrcode = m_muonEfficiencyScaleFactorsTool->getEfficiencyScaleFactor(**MuonShallowCopyItr, effSF);
    ...
    for( auto TauJetShallowCopyItr = (m_TauJetContShallowCopy.first)->begin(); TauJetShallowCopyItr != (m_TauJetContShallowCopy.first)->end(); TauJetShallowCopyItr++) {
      ...
      if(jobOptions_apply_tau_correction){
        if( m_tauSmearTool->applyCorrection((**TauJetShallowCopyItr)) != CP::CorrectionCode::Ok){
          Error("execute()","Cannot apply shallow-copy tau correction, eta = %.2f, pt = %.2f. Exiting...", (*TauJetShallowCopyItr)->eta(), (*TauJetShallowCopyItr)->pt() * MeV2GeV);
          return EL::StatusCode::FAILURE;
        }
      }
      ...
      //tau efficiency
      double effSF(0);
      m_tauEffTool->getEfficiencyScaleFactor((**TauJetShallowCopyItr), effSF);
      (*TauJetShallowCopyItr)->auxdata< double >( "jet_id_eff_sf" ) = static_cast< double >( effSF );

    //   int nCoreTracks = (*TauJetShallowCopyItr)->nTracks();
    //   for (int i=0;i<nCoreTracks;++i) {
    //      const xAOD::TrackParticle* trk = (*TauJetShallowCopyItr)->track(i);
    //   }
    //  
      //tau selection
    ...
    // Jets        |
    //--------------
  //  double      jet_JERes  = -999;
  //  double      jet_JERunc = -999;
    ...
    for(auto JetShallowCopyItr = (m_JetContShallowCopy.first)->begin(); JetShallowCopyItr != (m_JetContShallowCopy.first)->end(); JetShallowCopyItr++){
      ...
      if(jobOptions_apply_jet_recalibration){
        if( m_jetCalibTool->applyCalibration((**JetShallowCopyItr)) != CP::CorrectionCode::Ok){
          Error("execute()","Cannot apply shallow-copy jet calibration. Exiting...");
          return EL::StatusCode::FAILURE;
        }
      }
	...
	if(jobOptions_apply_JES_correction){
	  if( m_JESUncertaintyTool->applyCorrection(**JetShallowCopyItr) != CP::CorrectionCode::Ok ){
	    Error("execute()","Cannot apply shallow-copy JES correction. Exiting...");
	    return EL::StatusCode::FAILURE;
	  }
	}
        //apply JER correction
	if(jobOptions_apply_JER_correction){
	  if( m_JERSmearingTool->applyCorrection(**JetShallowCopyItr) != CP::CorrectionCode::Ok ){
	    Error("execute()","Cannot apply shallow-copy JER correction. Exiting...");
	    return EL::StatusCode::FAILURE;
	  }
	}
        /*
    ...
    */
    ...
    if(m_ViewElemTauCont->size() > 0 && m_ViewElemMuonCont->size() > 0){
      ...
      //FillTreeVar("leading_tau_muon_m_eff", m_bt.MassEffective(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) );
      //FillTreeVar("leading_tau_muon_m_col", m_bt.MassCollinear(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) );
