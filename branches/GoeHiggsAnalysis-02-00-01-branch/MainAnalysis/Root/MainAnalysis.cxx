...
EL::StatusCode MainAnalysis :: setupJob (EL::Job& job)
{
  ...
  // xJobOptions   ||
  //================
  //  m_jo = new xJobOptions(m_job_options_file);
 ...
 if( !m_jo->Configure() )
  ...
  //DAN: test dictionary capabilities
  TClass* class_dict = IsA();
  if(!class_dict) {
    std::cout << "DICT: something is wrong" << std::endl;
  } else {
    // list class members
    std::cout << "DICT: List public members of the class " << class_dict->GetName() << std::endl;
    TIter next(class_dict->GetListOfAllPublicDataMembers());
    TDataMember *p;
    while((p = (TDataMember*) next())) {
      TString idName     = TString(p->GetName());
      TString idTypeName = TString(p->GetTypeName());
      if( idTypeName == "bool" && idName.BeginsWith("property_")) {
        TString propertyName        = idName(9, idName.Length()-9);
        TString setterName        = TString("set_")+propertyName;
        TString defaultSetterName = TString("setDefault_")+propertyName;
        TString printoutName      = TString("print_")+propertyName;
        
        // try to retrieve the property
        TDataMember* property     = class_dict->GetDataMember(propertyName);
        if(!property) {
          std::cout << "  " << propertyName << " doesn't exist! Something is wrong" << std::endl;
        } else {
          TString typeName = property->GetTypeName();
          
          // call default setter
          Execute(defaultSetterName, "");
          
          // call a custom setter in case option is parsed from the text file: TODO
          if(propertyName=="test2") {
            Execute(setterName, "12345.12");
          }

          // printout          
          Execute(printoutName, "");
        }
      }
    }
  }
...
std::cout<<"gridflag "<<m_runGrid<<std::endl;
  ...
  //DAN View electrons
  m_ViewElemEleCont = new xAOD::ElectronContainer(SG::VIEW_ELEMENTS);
...
//mc event weight safety init - only called when vars not supposed to be used (ie data/xAOD)
  ...
  //DAN: included electron tools. Code taken from Tomas Davidek
  //DAN: electron selector?
  //============================
  ...
  m_egammaCalibrationAndSmearingTool->setProperty("ESModel", "es2012c");            //DAN: this should not be hardcoded
  m_egammaCalibrationAndSmearingTool->setProperty("ResolutionType", "SigmaEff90");
  if( !m_egammaCalibrationAndSmearingTool->initialize() ) {
  ...
  // Electron isolation
  //============================
  m_isoTool = CxxUtils::make_unique<CP::ElectronIsolationSelectionTool>("ElectronIsolationSelectionTool");
  if( !m_isoTool->initialize() ) {
    Error("initialize()", "Failed to properly initialize the ElectronIsolationSelectionTool Tool. Exiting." );
    return EL::StatusCode::FAILURE;
  }
  m_isoTool->configureCutBasedIsolation( xAOD::Iso::etcone20 , m_jo->get<double>(xJobOptions::ELE_ISO_ETCONE20), true );
  m_isoTool->configureCutBasedIsolation( xAOD::Iso::ptcone40 , m_jo->get<double>(xJobOptions::ELE_ISO_PTCONE40), true );  
...
EL::StatusCode MainAnalysis :: execute ()
{
  ...
  for (vitr = vertices->begin(); vitr != vertices->end(); ++vitr){
    if ( (*vitr)->nTrackParticles() > 1) m_npv++; //DAN: is this needed?
  }
  ...
  if ( !m_event->retrieve( JetCont, (m_jo->get<std::string>(xJobOptions::JET_COLLECTION)+"Jets").c_str() ).isSuccess() ){
    Error("execute()", "Failed to retrieve %s container. Exiting...", m_jo->get<std::string>(xJobOptions::JET_COLLECTION).c_str() ); //DAN fix
    return EL::StatusCode::FAILURE;
  }
  ...
  //DAN: list of cuts on electrons that define the good ones
  ...
  std::vector<std::string> vTauGoodFlags { "accepted", "no_muon_overlap", "no_ele_overlap" };

  //list of cuts on jets that define the good ones
  std::vector<std::string> vJetGoodFlags { "clean", "pt_cut", "eta_cut" , "no_muon_overlap", "no_ele_overlap", "no_tau_overlap", "jvf_cut"};
  ...
  //DAN: bugfix: moved ++nvar into the for-loop statement. Otherwise it doesn't get updated when "contine" is called from within the loop!
  ...
  for(std::vector<CP::SystematicSet>::const_iterator sysListItr = m_sysList.begin(); sysListItr != sysListItr_end; ++sysListItr, ++nvar){
    ...
    */
    ...
    // DAN: debug
    //Info("execute()", "Systematic: %s ", systName.c_str() );
    ...
    //DAN
    //Info("execute()", "Ele systematic");
    if( m_egammaCalibrationAndSmearingTool->applySystematicVariation( *sysListItr ) != CP::SystematicCode::Ok ) {
      Warning("execute()", "Cannot configure egamma calibration tool for systematic" );
      continue;
    }
    //Info("execute()", "...done");
    ...
    for(auto MuonShallowCopyItr = (m_MuonContShallowCopy.first)->begin(); MuonShallowCopyItr != (m_MuonContShallowCopy.first)->end(); MuonShallowCopyItr++) {
      ...
      //determine goodness
      //DAN: this seems dangerous since one has to make sure that "vMuonGoodFlags" flags are kept updated in case new selection is added
    ...
    // Electrons    |
    //---------------
    //DAN: electrons go after the muons
    m_egammaCalibrationAndSmearingTool->setDefaultConfiguration(eventInfo); // for electron momentum correction
    ...
    for(auto EleShallowCopyItr = (m_EleContShallowCopy.first)->begin(); EleShallowCopyItr != (m_EleContShallowCopy.first)->end(); EleShallowCopyItr++) {
      m_egammaCalibrationAndSmearingTool->setRandomSeed(eventInfo->eventNumber()+100*(EleShallowCopyItr-(m_EleContShallowCopy.first)->begin())); // Electron momentum corr: smearing
      ...
      if(m_jo->get<bool>(xJobOptions::ELE_CALIBRATION_SMEARING)) {
        if( m_egammaCalibrationAndSmearingTool->applyCorrection(*el, eventInfo) != CP::CorrectionCode::Ok) {
          Error("execute()","Cannot apply electron callibration and smearing, eta = %.2f, pt = %.2f. Exiting...", el->eta(), el->pt());
          return EL::StatusCode::FAILURE;
        }
      }
...
//        Error("execute()", "Selection menu \"" << m_jo->get<std::string>(xJobOptions::ELE_ID) << "\" is not available for electrons");
        return EL::StatusCode::FAILURE;
      }
      el->auxdata< int >( "accepted" ) = static_cast< int >(ele_id ? 1:0);
      el->auxdata< int >( "no_muon_overlap" ) = static_cast< int >(! (m_t->EleOverlapsWithGoodMuon( *EleShallowCopyItr,
      ...
      el->auxdata< int >( "iso_cut" )  = static_cast< int >( m_isoTool->accept(*el) ? 1:0 );
//      el->auxdata< int >( "iso_cut" )  = 1;  //FIXME
      ...
      // Note that looser cruriteria are required for overlap removal with taus
      ...
      //determine goodness
      el->auxdata< int >( "good" ) = static_cast< int >( m_t->Goodness(el, vEleGoodFlags)  );

    }
    // end for loop over electrons
    //Info("execute()", "...done");

    //Info("execute()", "Linking ele containers...");
    ...
    for( auto TauJetShallowCopyItr = (m_TauJetContShallowCopy.first)->begin(); TauJetShallowCopyItr != (m_TauJetContShallowCopy.first)->end(); TauJetShallowCopyItr++) {
      ...
      //DAN: tau-electron overlap
      (*TauJetShallowCopyItr)->auxdata< int >( "no_ele_overlap" ) = static_cast< int >( ! (m_t->TauOverlapsWithEle( *TauJetShallowCopyItr,
    ...
    for(auto JetShallowCopyItr = (m_JetContShallowCopy.first)->begin(); JetShallowCopyItr != (m_JetContShallowCopy.first)->end(); JetShallowCopyItr++){
      ...
      ( *JetShallowCopyItr )->auxdata< int >( "no_muon_overlap" ) = static_cast< int >( ! (m_t->JetOverlapsWithGoodMuon( *JetShallowCopyItr,
                                                                                                                          m_MuonContShallowCopy.first,
                                                                                                                          m_jo->get<double>(xJobOptions::MUON_JET_OVERLAP_DR)) ? 1:0) );
      
      //DAN: jet-electron overlap
      ( *JetShallowCopyItr )->auxdata< int >( "no_ele_overlap" ) = static_cast< int >( ! (m_t->JetOverlapsWithGoodEle( *JetShallowCopyItr,
                                                                                                                        m_EleContShallowCopy.first,
      ...
      ijet++; //DAN: this is not used anywhere in the loop. In any case, it would be safer to include it in the for statement
    ...
    //Info("execute()", "Creating view containers...");
    for(auto EleShallowCopyItr = (m_EleContShallowCopy.first)->begin(); EleShallowCopyItr != (m_EleContShallowCopy.first)->end(); EleShallowCopyItr++) {
    ...
    //Info("execute()", "Sorting...");
    m_t->PtSort(m_ViewElemEleCont);
    //Info("execute()", "...done");
    ...
    //electrons
    FillTreeVar("electrons_N", m_ViewElemEleCont->size());

    if(m_ViewElemEleCont->size() > 0){
      FillTreeVar("leading_electron", 1);
      FillTreeVar("leading_electron_pt",  m_ViewElemEleCont->at(0)->pt() * MeV2GeV );
      FillTreeVar("leading_electron_eta", m_ViewElemEleCont->at(0)->eta() );
      FillTreeVar("leading_electron_phi", m_ViewElemEleCont->at(0)->phi() );
      FillTreeVar("leading_electron_Q",  m_ViewElemEleCont->at(0)->charge() );
      FillTreeVar("leading_electron_met_mt", m_t->MT( m_ViewElemEleCont->at(0), MET_Calib) );
    }
    if(m_ViewElemEleCont->size() > 1){
      FillTreeVar("subleading_electron", 1);
      FillTreeVar("subleading_electron_pt",  m_ViewElemEleCont->at(1)->pt() * MeV2GeV);
      FillTreeVar("subleading_electron_eta", m_ViewElemEleCont->at(1)->eta() );
      FillTreeVar("subleading_electron_phi", m_ViewElemEleCont->at(1)->phi() );
      FillTreeVar("subleading_electron_Q", m_ViewElemEleCont->at(1)->charge() );
    }
    ...
    if(m_ViewElemTauCont->size() > 0 && m_ViewElemMuonCont->size() > 0){
      ...
      FillTreeVar("tau_muon_deta", m_t->DeltaEta(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) );
      FillTreeVar("tau_muon_dphi", m_t->DeltaPhi(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) );
      FillTreeVar("tau_muon_dR", m_t->DeltaR(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) );
      FillTreeVar("tau_muon_cosalpha", m_t->CosAlpha(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) );
      FillTreeVar("tau_muon_qxq", m_ViewElemTauCont->at(0)->charge() * m_ViewElemMuonCont->at(0)->charge() );
      FillTreeVar("tau_muon_m_vis", m_t->MassVisible(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) * MeV2GeV);
      FillTreeVar("tau_muon_vect_sum_pt", m_t->VectorSumPt(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) * MeV2GeV );
      FillTreeVar("tau_muon_scal_sum_pt", m_t->ScalarSumPt(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0) * MeV2GeV);
      FillTreeVar("tau_muon_met_bisect", m_t->METbisect(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0, MET_Calib) );
      FillTreeVar("tau_muon_met_min_dphi", m_t->METmindeltaphi(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0, MET_Calib) );
      FillTreeVar("tau_muon_met_centrality", m_t->METcentrality(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0, MET_Calib) );
      double x1_col; double x2_col; double m_col; bool ok_col = m_t->MassCollinear(m_ViewElemTauCont, 0, m_ViewElemMuonCont, 0, MET_Calib, true, m_col, x1_col, x2_col) ;
    ...
    if(m_ViewElemTauCont->size() > 0 && m_ViewElemEleCont->size() > 0){
      ...
      FillTreeVar("tau_electron_deta", m_t->DeltaEta(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0) );
      FillTreeVar("tau_electron_dphi", m_t->DeltaPhi(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0) );
      FillTreeVar("tau_electron_dR", m_t->DeltaR(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0) );
      FillTreeVar("tau_electron_cosalpha", m_t->CosAlpha(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0) );
      FillTreeVar("tau_electron_qxq", m_ViewElemTauCont->at(0)->charge() * m_ViewElemEleCont->at(0)->charge() );
      FillTreeVar("tau_electron_m_vis", m_t->MassVisible(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0) * MeV2GeV);
      FillTreeVar("tau_electron_vect_sum_pt", m_t->VectorSumPt(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0) * MeV2GeV );
      FillTreeVar("tau_electron_scal_sum_pt", m_t->ScalarSumPt(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0) * MeV2GeV);
      FillTreeVar("tau_electron_met_bisect", m_t->METbisect(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0, MET_Calib) );
      FillTreeVar("tau_electron_met_min_dphi", m_t->METmindeltaphi(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0, MET_Calib) );
      FillTreeVar("tau_electron_met_centrality", m_t->METcentrality(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0, MET_Calib) );
      double x1_col; double x2_col; double m_col; bool ok_col = m_t->MassCollinear(m_ViewElemTauCont, 0, m_ViewElemEleCont, 0, MET_Calib, true, m_col, x1_col, x2_col) ;
    ...
    // DAN muon electron system
    if(m_ViewElemMuonCont->size() > 0 && m_ViewElemEleCont->size() > 0){
      FillTreeVar("muon_electron", 1. );
      FillTreeVar("muon_electron_deta", m_t->DeltaEta(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0) );
      FillTreeVar("muon_electron_dphi", m_t->DeltaPhi(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0) );
      FillTreeVar("muon_electron_dR", m_t->DeltaR(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0) );
      FillTreeVar("muon_electron_cosalpha", m_t->CosAlpha(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0) );
      FillTreeVar("muon_electron_qxq", m_ViewElemMuonCont->at(0)->charge() * m_ViewElemEleCont->at(0)->charge() );
      FillTreeVar("muon_electron_m_vis", m_t->MassVisible(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0) * MeV2GeV);
      FillTreeVar("muon_electron_vect_sum_pt", m_t->VectorSumPt(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0) * MeV2GeV );
      FillTreeVar("muon_electron_scal_sum_pt", m_t->ScalarSumPt(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0) * MeV2GeV);
      FillTreeVar("muon_electron_met_bisect", m_t->METbisect(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0, MET_Calib) );
      FillTreeVar("muon_electron_met_min_dphi", m_t->METmindeltaphi(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0, MET_Calib) );
      FillTreeVar("muon_electron_met_centrality", m_t->METcentrality(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0, MET_Calib) );
      double x1_col; double x2_col; double m_col; bool ok_col = m_t->MassCollinear(m_ViewElemMuonCont, 0, m_ViewElemEleCont, 0, MET_Calib, true, m_col, x1_col, x2_col) ;
    ...
    if(m_ViewElemEleCont->size() > 1){
      ...
      FillTreeVar("dielectron_deta", m_t->DeltaEta(m_ViewElemEleCont) );
      FillTreeVar("dielectron_dphi", m_t->DeltaPhi(m_ViewElemEleCont) );
      FillTreeVar("dielectron_dR", m_t->DeltaR(m_ViewElemEleCont) );
      FillTreeVar("dielectron_cosalpha", m_t->CosAlpha(m_ViewElemEleCont) );
      FillTreeVar("dielectron_qxq", m_ViewElemEleCont->at(0)->charge() * m_ViewElemEleCont->at(1)->charge());
      FillTreeVar("dielectron_m_vis", m_t->MassVisible(m_ViewElemEleCont) * MeV2GeV);
      FillTreeVar("dielectron_vect_sum_pt", m_t->VectorSumPt(m_ViewElemEleCont) * MeV2GeV );
      FillTreeVar("dielectron_scal_sum_pt", m_t->ScalarSumPt(m_ViewElemEleCont) * MeV2GeV);
      FillTreeVar("dielectron_met_bisect", m_t->METbisect(m_ViewElemEleCont, MET_Calib) );
      FillTreeVar("dielectron_met_min_dphi", m_t->METmindeltaphi(m_ViewElemEleCont, MET_Calib) );
      FillTreeVar("dielectron_met_centrality", m_t->METcentrality(m_ViewElemEleCont, MET_Calib) );
      double x1_col; double x2_col; double m_col; bool ok_col = m_t->MassCollinear(m_ViewElemEleCont, MET_Calib, true, m_col, x1_col, x2_col) ;
...
std::cout<<"finalize"<<std::endl;
  ...
  m_egammaCalibrationAndSmearingTool.reset(); m_egammaCalibrationAndSmearingTool = nullptr; //DAN
  m_isoTool.reset(); m_isoTool = nullptr; //DAN
