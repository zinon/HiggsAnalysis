...
class xPlotterCanvas {
 ...
 public:
  ...
  void Canvas(const CanvasCasesT &t,
    ...
    //draw
    hEmpty1->Draw();
    int icol(stack_order.size()-1);
    for (auto itr = stack_order.rbegin(); itr != stack_order.rend(); ++itr, --icol){
    //    for ( auto &order : stack_order ){
      auto order = *itr;
      SetStack(hm[ forward_as_tuple(order, "NOMINAL", get<2>(t), get<1>(t), get<0>(t)) ], stack_colors[icol]);

      hm[ forward_as_tuple(order, "NOMINAL", get<2>(t), get<1>(t), get<0>(t)) ]->Draw("hist same");

      cout<<"drawing "<<order<<" : NOMINAL : "<< get<2>(t)<<":"<< get<1>(t) <<" : "<< get<0>(t)<<" logy "<<m_y_axis_1_log<<endl;
    ...
    cout<<"drawing data "<<hd[ forward_as_tuple( get<0>(t), get<1>(t), get<2>(t) ) ]<<" : NOMINAL : "<< get<0>(t)<<" : "<< get<1>(t) <<" : "<< get<2>(t) <<endl;
    cout<<"\t"<<hd[ forward_as_tuple(get<0>(t), get<1>(t), get<2>(t)) ]->GetName()<<endl;
    cout<<"\t"<<hd[ forward_as_tuple(get<0>(t), get<1>(t), get<2>(t)) ]->GetEntries()<<endl;

    SetData(hd[ forward_as_tuple( get<0>(t), get<1>(t), get<2>(t) ) ]);

    hd[ forward_as_tuple( get<0>(t), get<1>(t), get<2>(t) ) ]->Draw("EP same");

    //max y
    double maxim_y = std::max( href -> GetBinContent( href -> GetMaximumBin() ), 
			       hd[ forward_as_tuple(get<0>(t), get<1>(t), get<2>(t)) ] -> GetBinContent( hd[ forward_as_tuple(get<0>(t), get<1>(t), get<2>(t)) ] -> GetMaximumBin() ) );
    ...
    */
    ...
    //don't forget to reset and delete the shared_ptr's
